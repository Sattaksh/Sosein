...
previewContainer.innerHTML =     <div class="image-preview">     <img src="${imageSrc}" alt="${fileName}" title="${fileName}">     <button class="clear-image-btn" title="Remove image">×</button>     </div>     <div class="image-info">     📷 ${fileName} - Click × to remove     </div>    ;
...
results.innerHTML += `
  ...
  </div>`;  } else if (/actor|actress/i.test(wikiData.description)) {
// Top 3 Famous Works from Wiki data
const works = wikiData.extract.match(/known for[^.]+/i);
if (works) {
results.innerHTML +=    <div class="card">   <h3>🎬 Famous Works</h3>   <p>${works[0]}</p>   </div>;
}
}
...
results.innerHTML +=    <div class="card">   <h3>🎞 IMDb Info</h3>   <p><a href="${imdbSearchUrl}" target="_blank">🔗 Search "${term}" on IMDb</a></p>   </div>;
}
...
html = <div class="card">   <h3>🎬 IMDb Info</h3>   <p>Rating: ${enhance.imdbRating}</p>   <a href="${enhance.imdbLink}" target="_blank">View on IMDb</a>   </div>;
} else if (enhance.type === "actor") {
html = <div class="card">   <h3>🎭 Famous Works</h3>   <ul>${enhance.famousWorks.map(work => <li>${work}</li>).join("")}</ul>   <a href="${enhance.imdbLink}" target="_blank">View on IMDb</a>   </div>;
}
...
return   <div class="card">   <h2>${d.title || term}   <button class="speak-btn" data-title="${encodeURIComponent(d.title)}" title="Read aloud">🔊</button>   </h2>   <p class="wiki-summary">${d.extract || "No summary available."}</p>   ${img}   <div class="wiki-expand-control">   <button class="read-more-btn" data-title="${encodeURIComponent(d.title)}">Read more</button>   <button class="collapse-btn" style="display:none;">Collapse</button>   </div>   <div class="expanded-content" style="display:none; margin-top: 1rem;"></div>   <br><a href="${d.content_urls.desktop.page}" target="_blank">Read on Wikipedia</a>   </div>  ;
}
...
const bookCard =     <div class="card">     <h2>📚 ${title}</h2>     <p><strong>Author(s):</strong> ${authors}</p>     <p>${description}</p>     ${thumbnail ?<img src="${thumbnail}" alt="Book Cover">: ""}     <br><a href="${previewLink}" target="_blank">🔗 Preview Book</a>     </div>    ;
...
const suggestionHTML =     <div class="card" style="background:#fff3cd; border-left: 4px solid #ffc107;">     <p>🤔 Did you mean      <a href="#" id="didYouMeanLink">${firstSuggestion}</a>?     </p>     </div>    ;
